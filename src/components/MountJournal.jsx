...
import {
  FaCheck, FaTimes, FaTools, FaClipboardCheck, FaChevronDown, FaChevronUp,
  FaCommentDots, FaConciergeBell, FaTruckLoading, FaSpinner, FaBoxOpen
} from 'react-icons/fa';

// СПІЛЬНИЙ ЖУРНАЛ МОНТАЖУ: список специфікації з прогресом,
// звіт "встановлено сьогодні" (з автовидачею з резерву),
// експрес-прийомка прямої поставки, фіксація послуг.
// Використовується в DealInstallation та DealCrewSchedule.
export default function MountJournal({ dealId, onProgressUpdate, onStatsChange }) {
  const { employeeProfile } = useAuth();

  const [bomItems, setBomItems] = useState([]);
  const [loading, setLoading] = useState(true);

  // Модалка: фіксація монтажу
  ...
  // Модалка: експрес-прийомка прямої поставки
  ...
  const fetchBom = async () => {
    ...
    const mountedCount = items.filter(item => Number(item.quantity_mounted || 0) >= Number(item.quantity_planned || 0)).length;
    if (onProgressUpdate) onProgressUpdate({ total: totalPlanned, mounted: mountedCount });
    if (onStatsChange) onStatsChange({
      total: totalPlanned,
      mounted: mountedCount,
      allMounted: totalPlanned > 0 && mountedCount === totalPlanned
    });
  ...
  if (loading) {
    return <div className="py-12 text-center text-slate-400 font-black uppercase text-xs animate-pulse bg-white rounded-2xl border border-slate-200">Завантаження журналу...</div>;
  }
