...
import {
  FaTimes, FaUser, FaBuilding, FaMoneyBillWave, FaEdit, FaTrash, FaSave,
  FaCheckCircle, FaExclamationCircle, FaArrowRight
} from 'react-icons/fa';

export default function PaymentDetailsModal({ isOpen, onClose, payment, debtStatus, onSaveSuccess, onOpenDeal }) {
  ...
  useEffect(() => {
    if (isOpen && payment) {
      setForm({
        ...
        payment_method: payment.payment_method || 'Готівка',
        payment_category: payment.payment_category || 'Аванс',
        notes: payment.notes || ''
      });
      setIsEditing(false);
    ...
    <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in custom-scrollbar overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col my-auto overflow-hidden">
        ...
        <div className="p-6 space-y-5 bg-slate-50/50">
          ...
          {payment.deals && (
            <div
              onClick={() => onOpenDeal && onOpenDeal(payment.deals.id)}
            ...
            >
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Угода</p>
                <p className="font-bold text-slate-800 text-sm mt-0.5">№{payment.deals.custom_id} — {payment.deals.title}</p>
              </div>
          ...
          {debtStatus && (
            <div className="flex justify-center">
              {debtStatus.isPaid ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-[10px] font-black uppercase tracking-widest border border-emerald-100"><FaCheckCircle size={12} /> Угода оплачена повністю</span>
              ) : (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 text-rose-700 rounded-lg text-[10px] font-black uppercase tracking-widest border border-rose-100"><FaExclamationCircle size={12} /> Борг по угоді: {debtStatus.debt.toLocaleString()} $</span>
              )}
            </div>
          )}
            ...
            <div className="bg-white p-4 rounded-2xl border border-amber-200 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.payment_category} onChange={e => setForm({ ...form, payment_category: e.target.value })} className="w-full px-2 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold outline-none focus:border-amber-500 cursor-pointer">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-black text-emerald-600 text-lg mt-0.5">+{Number(payment.amount_usd).toLocaleString()} $</p>
                  <p className="text-[10px] font-bold text-slate-400 mt-0.5">{Number(payment.amount_uah).toLocaleString()} ₴ (курс {payment.exchange_rate})</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Дата й час</p>
                  <p className="font-bold text-slate-800 text-sm mt-0.5">{pDate.toLocaleDateString('uk-UA')}</p>
                  <p className="text-[10px] font-bold text-slate-400">{pDate.toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <span className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-md text-[9px] font-black uppercase tracking-widest">{payment.payment_method}</span>
                <span className="px-2.5 py-1 bg-amber-50 text-amber-800 border border-amber-100 rounded-md text-[9px] font-black uppercase tracking-widest">{payment.payment_category}</span>
              </div>
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Менеджер</p>
                <p className="font-bold text-slate-700 text-sm mt-0.5">{payment.users?.full_name || 'Система'}</p>
              </div>
              {payment.notes && (
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Коментар</p>
                  <p className="text-sm font-medium text-slate-700 mt-0.5">{payment.notes}</p>
                </div>
              )}
            </div>
          )}
