...
export default function FinanceDashboard() {
  ...
  // ДОДАНО: Стейт для фільтру по датах
    ...
    return deals.filter(d => {
      ...
      if (dateRange.start || dateRange.end) {
        ...
        if (dateRange.end) {
           ...
           endDate.setHours(23, 59, 59, 999); // Включаємо весь останній день
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 min-h-screen pb-10 relative">
      
      {/* ХЕДЕР ТА ФІЛЬТРИ */}
      <div className="bg-white px-6 py-5 border-b border-slate-200 sticky top-0 z-20 flex flex-col gap-4 shadow-sm">
        
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-slate-900 text-emerald-400 rounded-xl shadow-md"><FaChartLine size={20}/></div>
            <div>
              <h1 className="text-xl font-black text-slate-900 uppercase tracking-tight">Фінансова Аналітика</h1>
            </div>
          </div>
      ...
      {/* KPI КАРТКИ */}
      <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                  ...
                  <th className="py-4 px-4 text-right">Бюджет (Дохід)</th>
                      ...
                      <table className="w-full text-left whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {bomDetails.map((item) => {
                            ...
                            // ВИКОРИСТОВУЄМО УНІФІКОВАНІ ПОЛЯ В USD!
